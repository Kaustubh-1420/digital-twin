...
export default function AvatarViewer({ glbUrl, loading, landmarksRef, normLandmarksRef, leftHandRef, rightHandRef, mirrorRef, active }: Props) {
  return (
    <div className="flex flex-col gap-2 flex-1 min-h-0">
      ...
      <div className="flex-1 min-h-0 rounded-xl overflow-hidden border border-zinc-800">
        <AvatarCanvas glbUrl={glbUrl} loading={loading} landmarksRef={landmarksRef} normLandmarksRef={normLandmarksRef} leftHandRef={leftHandRef} rightHandRef={rightHandRef} mirrorRef={mirrorRef} webcamActive={active} />
      </div>
    </div>
  );
}
